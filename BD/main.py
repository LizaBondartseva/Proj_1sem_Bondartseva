...
class Main(tk.Frame):
    ...
    def search_record(self, search):
        self.db.cur.execute("""UPDATE users SET user_id=?, name=?, sex=?, old=?, score=? WHERE user_id=?""",
                            (search, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.con.commit()
...
class Srch(Search):
    ...
    def init_edit(self):
        ...
        btn_search.place(x=205, y=70)
        btn_search.bind('<Button-1>', lambda event: self.view.update_record(self.entry_description.get(),
                                                                          self.entry_name.get(),
                                                                          self.combobox.get(),
                                                                          self.entry_old.get(),
                                                                          self.entry_score.get()))
